...
void CcaInteractiveGoals::onInitialize()
{
  ...
  // std::shared_ptr<interactive_marker_manager::InteractiveMarkerManager> imm;
  // this = std::make_shared<interactive_marker_manager::InteractiveMarkerManager>();
  ...
  // this->enableInteractiveMarkerControls("arrow_marker", interactive_marker_manager::ImControlEnable::ALL, true);
  this->hideInteractiveMarker("arrow_marker");
...
req_.task_description.affordance_info.pitch = std::stof(pitch_value_input_->text().toStdString());
  ...
  // Get affordance pose
    ...
    auto screw_info = this->getAffordancePose_(
...
void CcaInteractiveGoals::motionTypeSelected(int index)
{
      ...
      motion_type_combo_box_->currentText() == "Screw Motion")
  {
    ...
    this->enableInteractiveMarkerControls("arrow_marker", interactive_marker_manager::ImControlEnable::ALL);
  }
  else
  {
    conf_place_button_->setEnabled(false);
...
// };
  ...
  if (index != 0)
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Calling DRAWWWWWWWWW");
  this->drawEeOrControlIm(index);
