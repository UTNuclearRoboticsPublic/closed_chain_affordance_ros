namespace cca_ros_action
{
    CcaRosActionServer::CcaRosActionServer(const std::string &node_name, const rclcpp::NodeOptions &node_options)
        : cca_ros::CcaRos(node_name, node_options), as_server_name_("/cca_ros_action"), timeout_secs_(60)
    {
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Received a new goal request on action server '%s'.", as_server_name_.c_str());
        (void)uuid;
    ...
    void CcaRosActionServer::handle_accepted(const std::shared_ptr<GoalHandleCcaRosAction> goal_handle)
    {
        ...
        std::thread{std::bind(&CcaRosAction::execute_action, this, std::placeholders::_1), goal_handle}.detach();
    }
    ...
    void CcaRosActionServer::execute_action(const std::shared_ptr<GoalHandleCcaRosAction> goal_handle)
    {
        ...
        PlanningRequestPtr req = goal->cca_planning_request;
